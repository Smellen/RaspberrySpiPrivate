	...
	while(true){
		...
		if(frameCount == 5){
			...
			if(whiteCount >= 1000){
				//time_t now = time(0);
				//cout << now << endl;
				imwrite ("/var/www/RaspberrySpi/photos/motionPictures/difference.png", difference );
				imwrite ("/var/www/RaspberrySpi/photos/motionPictures/prevFrame.png", prevFrame );
				imwrite ("/var/www/RaspberrySpi/photos/motionPictures/currFrame.png", currFrame );
